...
void UISubsystem::OnInitialize(Engine *owner) {
  ...
  // Now safe to initialize directly as Engine guarantees RenderSubsystem is
  // ready (if registered before UI)
  InitializeImGui();

  if (m_initialized) {
    Logger::Info("UISubsystem", "UI Subsystem Initialized.");
  } else {
    Logger::Error("UISubsystem", "UI Subsystem failed to initialize ImGui.");
  }

  // Integrate with SceneEditorSubsystem
  SceneEditorSubsystem *sceneEditor =
      m_owner->GetSubsystem<SceneEditorSubsystem>();
  if (sceneEditor) {
    Logger::Info("UISubsystem",
                 "SceneEditorSubsystem integration established.");
  }
}
...
void UISubsystem::InitializeImGui() {
  ...
  if (!platform || !renderer) {
    Logger::Critical("UISubsystem",
                     "Platform or Render subsystem not available!");
    return;
  }
  ...
  if (vkCreateDescriptorPool(vulkanDevice->GetVkDevice(), &pool_info, nullptr,
  ...
  if (vkCreateCommandPool(vulkanDevice->GetVkDevice(), &cmd_pool_info, nullptr,
                          &m_uiCommandPool) != VK_SUCCESS) {
  ...
  // init_info.CommandPool = m_uiCommandPool; // Removed: Not supported in this
  // ImGui version
  ...
  // We need static storage for the format because pColorAttachmentFormats is a
  // pointer
  ...
  m_initialized = true;
  Logger::Info("UISubsystem",
               "ImGui Initialized with SDL3 and Vulkan backends.");
#endif
}
...
void UISubsystem::ShutdownImGui() {
  ...
  if (rhiDevice) {
    ...
    if (m_descriptorPool != VK_NULL_HANDLE) {
      vkDestroyDescriptorPool(vulkanDevice->GetVkDevice(), m_descriptorPool,
                              nullptr);
      m_descriptorPool = VK_NULL_HANDLE;
    }
    if (m_uiCommandPool != VK_NULL_HANDLE) {
      vkDestroyCommandPool(vulkanDevice->GetVkDevice(), m_uiCommandPool,
                           nullptr);
