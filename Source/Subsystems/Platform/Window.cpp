...
namespace std {
namespace chrono {
template <typename Rep> class duration { /* ... */
};
} // namespace chrono
} // namespace std
#endif
...
bool Window::Initialize(const std::string &title, int width, int height) {
  ...
  Logger::Info("Window", "Initializing window: '{}' ({}x{})", title, width,
               height);

  // Security validation
  if (width < 100 || height < 100) {
    Logger::Error("Window",
                  "Invalid window dimensions: minimum 100x100 required");
    return false;
  }
  ...
  // Log the video driver
  const char *driver = SDL_GetCurrentVideoDriver();
  Logger::Info("Window", "Current SDL Video Driver: {}",
               driver ? driver : "Unknown");

  // SDL3 window creation with Vulkan fallback
  ...
  if (!m_data->SDLWindow) {
    Logger::Warning(
        "Window",
        "Vulkan window creation failed: {}. Retrying without Vulkan flag...",
        SDL_GetError());
    m_data->SDLWindow = SDL_CreateWindow(
        title.c_str(), width, height, SDL_WINDOW_RESIZABLE | SDL_WINDOW_HIDDEN);
  }
  ...
  if (actualWidth != width || actualHeight != height) {
    Logger::Warning("Window", "Requested size {}x{}, actual size {}x{}", width,
  ...
  Logger::Warning("Window",
                  "SDL3 not available - using placeholder implementation");
...
void Window::SwapBuffers() {
  ...
  Logger::Warning(
      "Window",
      "SwapBuffers called - Vulkan doesn't need traditional buffer swapping");
...
void Window::SetFullscreen(bool enabled) {
  ...
  Logger::Info("Window", "Fullscreen mode {}",
...
bool Window::CreateRenderContext() {
  ...
  // Note: For Vulkan-based rendering, we don't need to create a traditional
  // render context Vulkan manages its own device and context through the
  // VulkanInstance and VulkanDevice This method is kept for compatibility but
  // will log a warning

  Logger::Warning("Window", "CreateRenderContext called - Vulkan doesn't need "
                            "traditional render context");
  Logger::Warning("Window",
                  "Use CreateVulkanSurface for Vulkan rendering instead");

  // Mark as created to avoid repeated warnings, but this is not actually used
  // for Vulkan
...
void Window::DestroyRenderContext() {
  ...
  // Note: For Vulkan-based rendering, we don't need to destroy a traditional
  // render context Vulkan manages its own device and context cleanup
  Logger::Debug("Window",
                "DestroyRenderContext called - no action needed for Vulkan");
...
std::vector<const char *> Window::GetRequiredVulkanExtensions() const {
  ...
  if (!m_data->IsInitialized || !m_data->SDLWindow) {
    Logger::Error("Window",
                  "Window not initialized - cannot get Vulkan extensions");
    return extensions;
  }
  ...
  if (!sdlExtensions || extensionCount == 0) {
    Logger::Error("Window", "Failed to get Vulkan extensions from SDL3: {}",
                  SDL_GetError());
    return extensions;
  }
  ...
  Logger::Info("Window", "Found {} required Vulkan extensions",
               extensions.size());

#else
  Logger::Warning("Window",
                  "SDL3 not available - no Vulkan extensions will be added");
#endif
...
bool Window::CreateVulkanSurface(VkInstance instance,
  ...
  if (!SDL_Vulkan_CreateSurface(m_data->SDLWindow, instance, nullptr,
                                surface)) {
    Logger::Error("Window", "Failed to create Vulkan surface: {}",
                  SDL_GetError());
    return false;
  }
...
void Window::HandleKeyboardEvent(const void *event) {
  ...
  case SDL_EVENT_KEY_DOWN: {
    m_data->inputManager->HandleSDLKeyEvent(sdlEvent->key.key, true,
                                            sdlEvent->key.repeat != 0);
    if (!sdlEvent->key.repeat) {
      Logger::Trace("Window", "Key pressed: {} (scancode: {})",
                    SDL_GetKeyName(sdlEvent->key.key),
  ...
  case SDL_EVENT_KEY_UP: {
    ...
    Logger::Trace("Window", "Key released: {}",
                  SDL_GetKeyName(sdlEvent->key.key));
    break;
  }
...
void Window::HandleMouseEvent(const void *event) {
  ...
  case SDL_EVENT_MOUSE_BUTTON_DOWN: {
    m_data->inputManager->HandleSDLMouseButtonEvent(
        sdlEvent->button.button, true, sdlEvent->button.x, sdlEvent->button.y);
    Logger::Trace("Window", "Mouse button {} pressed at ({}, {})",
                  sdlEvent->button.button, sdlEvent->button.x,
                  sdlEvent->button.y);
    break;
  }
  ...
  case SDL_EVENT_MOUSE_BUTTON_UP: {
    m_data->inputManager->HandleSDLMouseButtonEvent(
        sdlEvent->button.button, false, sdlEvent->button.x, sdlEvent->button.y);
  ...
  case SDL_EVENT_MOUSE_MOTION: {
    m_data->inputManager->OnMouseMoveEvent(
        static_cast<int>(sdlEvent->motion.x),
        static_cast<int>(sdlEvent->motion.y));
    Logger::Trace("Window",
                  "Mouse moved to ({:.1f}, {:.1f}), delta: ({:.1f}, {:.1f})",
                  sdlEvent->motion.x, sdlEvent->motion.y, sdlEvent->motion.xrel,
                  sdlEvent->motion.yrel);
    break;
  }

  case SDL_EVENT_MOUSE_WHEEL: {
    m_data->inputManager->OnMouseWheelEvent(sdlEvent->wheel.x,
                                            sdlEvent->wheel.y);
    Logger::Debug("Window", "Mouse wheel: Y={:.1f}, X={:.1f}",
                  sdlEvent->wheel.y, sdlEvent->wheel.x);
    break;
  }
...
void Window::HandleOtherEvent(const void *event) {
  ...
  case SDL_EVENT_DROP_FILE: {
    ...
    if (droppedFile) {
      Logger::Info("Window", "File dropped: {} at ({:.1f}, {:.1f})",
                   droppedFile, sdlEvent->drop.x, sdlEvent->drop.y);

      auto &eventManager = EventManager::GetInstance();
      eventManager.PublishEvent<FileDropEvent>(droppedFile, sdlEvent->drop.x,
                                               sdlEvent->drop.y);
    }
    break;
  }
 ...
 *
 * Converts SDL3 events to engine event system and routes them to appropriate
 * components. This method is called every frame and processes all window,
...
// Platform specific handle alma
    ...
    void *x11_window = SDL_GetPointerProperty(
        props, SDL_PROP_WINDOW_X11_WINDOW_POINTER, nullptr);
    if (x11_window)
      return x11_window;
